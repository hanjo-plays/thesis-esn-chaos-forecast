...
MODEL_FILE = PROJECT_ROOT / "models" / "best_ffnn_w10.h5"
SCALER_FILE = PROJECT_ROOT / "models" / "scaler_ffnn_w10.pkl"
...
def autoregressive_forecast_w10_ffnn(model, scaler, window_orig, steps):
    preds = []
    last_window = window_orig.copy()
    t_fwd = None

    for k in range(steps):
        # flatten the 10-step window and normalize it for the model
        last_flat = last_window.reshape(1, -1)
        last_scaled = scaler.transform(last_flat)

        # predict next [x, y, z] from the window
        with tf.device("/cpu:0"):
            t0 = time.time()
            y_hat = model.predict(last_scaled, verbose=0)[0]
            dt = time.time() - t0
...
print(f"\n--- Running Autoregressive Forecast from index {START_IDX} for {FORECAST_STEPS} steps (W={WINDOW}) ---")
...
pred_traj, fwd_time = autoregressive_forecast_w10_ffnn(
    best_model, scaler, window_orig, steps=FORECAST_STEPS
)
...
print("\n========== MODEL COMPLEXITY ==========")
print(f"Trainable parameters (FFNN-W10): {trainable_params}")
...
np.save(PROJECT_ROOT / "models" / "ffnn_w10_error_curve.npy", errors)
print("\nSaved error curve: ffnn_w10_error_curve.npy")
...
fig.tight_layout()
fig.savefig(FIGURES_DIR / "Forecast_FFNN10_fig0.png", dpi=300)
plt.close(fig)
...
fig1.tight_layout()
fig1.savefig(FIGURES_DIR / "Forecast_FFNN10_fig1.png", dpi=300)
plt.close(fig1)
print("Figures saved.")
