...
mean = inputs[:train_size].mean(axis=0)
std = inputs[:train_size].std(axis=0)

# Normalize targets with the same stats since it's the same Lorenz coordinates
...
def train_esn(u_train, y_train, params, rng_seed=42):
    ...
    # Rescale so spectral radius matches 
    # This controls how long info persists
...
save_path = MODELS_DIR / "best_esn_w1.pkl"
